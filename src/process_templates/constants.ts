// PROCESS TEMPLATE CONTROLLER MESSAGE PATTERNS
export const MICROSERVICE_CREATE_PROCESS_TEMPLATE_PATTERN =
  'v0.1_create_process_template';
export const MICROSERVICE_FIND_ALL_PROCESS_TEMPLATE_PATTERN =
  'v0.1_find_all_process_templates';
export const MICROSERVICE_FIND_ONE_PROCESS_TEMPLATE_PATTERN =
  'v0.1_find_one_process_template';
export const MICROSERVICE_UPDATE_PROCESS_TEMPLATE_PATTERN =
  'v0.1_update_process_template';
export const MICROSERVICE_REMOVE_PROCESS_TEMPLATE_PATTERN =
  'v0.1_remove_process_template';
export const MICROSERVICE_DEACTIVATE_PROCESS_TEMPLATE_PATTERN =
  'v0.1_deactivate_process_template';
export const MICROSERVICE_VALIDATE_PROCESS_TEMPLATE_CONTEXT_PATTERN =
  'v0.1_validate_process_template_context';

export const MESSAGE_BROKER_PROCESS_TEMPLATE_SERVICE_CLIENT_TOKEN =
  'process_template_service_token';
