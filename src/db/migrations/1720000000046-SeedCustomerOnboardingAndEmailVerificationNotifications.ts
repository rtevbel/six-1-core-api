...
/**
 * Seeds global notification templates and rules for customer onboarding
 * and tenant email verification (Handlebars / NV paths).
 */
export class SeedCustomerOnboardingAndEmailVerificationNotifications1720000000046
  implements MigrationInterface
{
  name = 'SeedCustomerOnboardingAndEmailVerificationNotifications1720000000046';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    for (const template of PLATFORM_NOTIFICATION_TEMPLATE_SEED) {
    ...
    for (const rule of PLATFORM_NOTIFICATION_RULE_SEED) {
      const templateId = templateIdByKey.get(rule.templateKey);
      if (!templateId) {
        throw new Error(
          `SeedCustomerOnboardingAndEmailVerificationNotifications1720000000046: missing template ${rule.templateKey}`,
        );
      }
    ...
    for (const entry of PLATFORM_NOTIFICATION_RULE_SEED) {
      await queryRunner.query(
        `
        INSERT INTO \`events\` (
          \`name\`,
        ...
        `,
        [entry.eventName, `Platform event ${entry.eventName}`, systemUserId, entry.eventName],
      );
    }
  }
  ...
  private async ensureEmailChannel(
    ...
    if (!rows[0]?.channel_id) {
      throw new Error(
        'SeedCustomerOnboardingAndEmailVerificationNotifications1720000000046: email channel missing',
      );
    }
  ...
  private async resolveSystemUserId(queryRunner: QueryRunner): Promise<number> {
    ...
    if (!fallback[0]?.user_id) {
      throw new Error(
        'SeedCustomerOnboardingAndEmailVerificationNotifications1720000000046: no users row available',
      );
    }
