...
export class ScheduledTasksService {
  ...
  async markQueued(
    ...
    await this.repo.update(
      { scheduledTaskId: id },
      { status: 'queued', startJobId, endJobId },
    );
  }
  ...
  async markRunning(id: number, when: Date) {
    await this.repo.update(
      { scheduledTaskId: id },
      { actualStartUtc: when, status: 'running' },
    );
  }
  ...
  async markCompleted(id: number, when: Date) {
    await this.repo.update(
      { scheduledTaskId: id },
      { actualEndUtc: when, status: 'completed', isActive: 0 },
    );
  }
  ...
  async pauseUntil(
    ...
    await this.repo.update(
      { scheduledTaskId: id },
      { blockedUntilUtc: untilUtc, blockReason: reason, status: 'paused' },
    );
  }
  ...
  async resume(id: number) {
    await this.repo.update(
      { scheduledTaskId: id },
      { blockedUntilUtc: null, blockReason: 'none', status: 'scheduled' },
    );
  }
